...
view(roses)
unique(roses)
roses_l <-  str_to_lower(str_sub(roses))
...
English <- mutate(English_letter, letter = rownames(English_letter)) %>%
  arrange(EnglishLetterFrequency)
...
code_lookup <- code %>%
  select(Var1, letter)
coding <- tapply(code_lookup$letter, code_lookup$Var1, unique)
decode <- data.frame(code = rose_split$letter,
                    decoded = (coding[rose_split$letter]))
frequon(subject = "Re: Transcription", content = decode)
